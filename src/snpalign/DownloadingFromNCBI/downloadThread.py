...
class DownloadThread(threading.Thread):
    ...
    def run(self):
        try:
            print("url",self.query + self.ID + self.retType, self.pathAndFileName)
            urllib.request.urlretrieve(self.query + self.ID + self.retType, filename=self.pathAndFileName)
